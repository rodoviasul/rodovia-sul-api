...
function axis(orient, scale3) {
  var tickArguments = [], tickValues = null, tickFormat = null, tickSizeInner = 6, tickSizeOuter = 6, tickPadding = 3, offset2 = typeof window !== "undefined" && window.devicePixelRatio > 1 ? 0 : 0.5, k2 = orient === top || orient === left ? -1 : 1, x3 = orient === left || orient === right ? "x" : "y", transform2 = orient === top || orient === bottom ? translateX : translateY;
  function axis2(context) {
    var values2 = tickValues == null ? scale3.ticks ? scale3.ticks.apply(scale3, tickArguments) : scale3.domain() : tickValues, format3 = tickFormat == null ? scale3.tickFormat ? scale3.tickFormat.apply(scale3, tickArguments) : identity_default : tickFormat, spacing = Math.max(tickSizeInner, 0) + tickPadding, range3 = scale3.range(), range0 = +range3[0] + offset2, range1 = +range3[range3.length - 1] + offset2, position3 = (scale3.bandwidth ? center : number)(scale3.copy(), offset2), selection2 = context.selection ? context.selection() : context, path2 = selection2.selectAll(".domain").data([null]), tick = selection2.selectAll(".tick").data(values2, scale3).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line2 = tick.select("line"), text2 = tick.select("text");
...
function data_default(value, key) {
  ...
  for (var m = groups.length, update = new Array(m), enter = new Array(m), exit = new Array(m), j = 0; j < m; ++j) {
    var parent = parents[j], group3 = groups[j], groupLength = group3.length, data = arraylike(value.call(parent, parent && parent.__data__, j, parents)), dataLength = data.length, enterGroup = enter[j] = new Array(dataLength), updateGroup = update[j] = new Array(dataLength), exitGroup = exit[j] = new Array(groupLength);
...
function ringContains(ring, point6) {
  ...
  for (var i = 0, n = ring.length, j = n - 1; i < n; j = i++) {
    ...
    if (segmentContains(pi7, pj, point6)) return 0;
...
function contours_default() {
  ...
  function isorings(values2, value, callback) {
    ...
    x3 = y3 = -1;
    t1 = above(values2[0], value);
    cases[t1 << 1].forEach(stitch);
    while (++x3 < dx - 1) {
      t0 = t1, t1 = above(values2[x3 + 1], value);
      cases[t0 | t1 << 1].forEach(stitch);
    }
    cases[t1 << 0].forEach(stitch);
    while (++y3 < dy - 1) {
      x3 = -1;
      t1 = above(values2[y3 * dx + dx], value);
      t2 = above(values2[y3 * dx], value);
      cases[t1 << 1 | t2 << 2].forEach(stitch);
      while (++x3 < dx - 1) {
        t0 = t1, t1 = above(values2[y3 * dx + dx + x3 + 1], value);
        t3 = t2, t2 = above(values2[y3 * dx + x3 + 1], value);
        cases[t0 | t1 << 1 | t2 << 2 | t3 << 3].forEach(stitch);
      }
      cases[t1 | t2 << 3].forEach(stitch);
    }
    x3 = -1;
    t2 = values2[y3 * dx] >= value;
    ...
    function stitch(line2) {
      var start2 = [line2[0][0] + x3, line2[0][1] + y3], end = [line2[1][0] + x3, line2[1][1] + y3], startIndex = index(start2), endIndex = index(end), f, g;
...
function density_default() {
  var x3 = defaultX, y3 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m = dy + o * 2 >> k2, threshold2 = constant_default5(20);
  function grid(data) {
  ...
  density2.x = function(_) {
    return arguments.length ? (x3 = typeof _ === "function" ? _ : constant_default5(+_), density2) : x3;
  };
  density2.y = function(_) {
    return arguments.length ? (y3 = typeof _ === "function" ? _ : constant_default5(+_), density2) : y3;
  };
...
var Voronoi = class {
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    while (true) {
      if (c0 === 0 && c1 === 0) return [x06, y06, x12, y12];
  ...
  function parseRows(text2, f) {
    ...
    function token() {
      ...
      if (text2.charCodeAt(j) === QUOTE) {
        ...
        if ((i = I) >= N) eof = true;
        else if ((c4 = text2.charCodeAt(I++)) === NEWLINE) eol = true;
        else if (c4 === RETURN) {
          eol = true;
      ...
      while (I < N) {
        if ((c4 = text2.charCodeAt(i = I++)) === NEWLINE) eol = true;
        else if (c4 === RETURN) {
          eol = true;
          if (text2.charCodeAt(I) === NEWLINE) ++I;
...
function add(tree2, x3, y3, d) {
  ...
  while (node.length) {
    ...
    if (parent = node, !(node = node[i = bottom2 << 1 | right2])) return parent[i] = leaf, tree2;
  }
  xp = +tree2._x.call(null, node.data);
  yp = +tree2._y.call(null, node.data);
  if (x3 === xp && y3 === yp) return leaf.next = node, parent ? parent[i] = leaf : tree2._root = leaf, tree2;
  do {
...
function circleStream(stream, radius2, delta, direction, t0, t1) {
  if (!delta) return;
  var cosRadius = cos2(radius2), sinRadius = sin2(radius2), step = direction * delta;
  if (t0 == null) {
    t0 = radius2 + direction * tau4;
    t1 = radius2 - step / 2;
  } else {
...
function circleRadius(cosRadius, point6) {
  ...
  return ((-point6[2] < 0 ? -radius2 : radius2) + tau4 - epsilon5) % tau4;
}
...
function circle_default(radius2) {
  var cr = cos2(radius2), delta = 6 * radians, smallRadius = cr > 0, notHemisphere = abs3(cr) > epsilon5;
  ...
  function intersect(a2, b, two) {
    ...
    var delta2 = lambda1 - lambda0, polar = abs3(delta2 - pi3) < epsilon5, meridian = polar || delta2 < epsilon5;
    if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
    if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs3(q[0] - lambda0) < epsilon5 ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : delta2 > pi3 ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
  ...
  return clip_default(visible, clipLine, interpolate, smallRadius ? [0, -radius2] : [-pi3, radius2 - pi3]);
}
...
function path_default2(projection3, context) {
  ...
  path2.projection = function(_) {
    ...
    projectionStream = _ == null ? (projection3 = null, identity_default2) : (projection3 = _).stream;
    return path2;
  };
...
function resample(project2, delta2) {
  function resampleLineTo(x06, y06, lambda0, a0, b02, c0, x12, y12, lambda1, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b02 + b12, c4 = c0 + c1, m = sqrt(a2 * a2 + b * b + c4 * c4), phi2 = asin(c4 /= m), lambda2 = abs3(abs3(c4) - 1) < epsilon5 || abs3(lambda0 - lambda1) < epsilon5 ? (lambda0 + lambda1) / 2 : atan2(b, a2), p = project2(lambda2, phi2), x22 = p[0], y22 = p[1], dx2 = x22 - x06, dy2 = y22 - y06, dz = dy * dx2 - dx * dy2;
...
function projectionMutator(projectAt) {
  var project2, k2 = 150, x3 = 480, y3 = 250, lambda = 0, phi2 = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default2, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function albersUsa_default() {
  ...
  albersUsa.translate = function(_) {
    ...
    alaskaPoint = alaska.translate([x3 - 0.307 * k2, y3 + 0.201 * k2]).clipExtent([[x3 - 0.425 * k2 + epsilon5, y3 + 0.12 * k2 + epsilon5], [x3 - 0.214 * k2 - epsilon5, y3 + 0.234 * k2 - epsilon5]]).stream(pointStream);
    hawaiiPoint = hawaii.translate([x3 - 0.205 * k2, y3 + 0.212 * k2]).clipExtent([[x3 - 0.214 * k2 + epsilon5, y3 + 0.166 * k2 + epsilon5], [x3 - 0.115 * k2 - epsilon5, y3 + 0.234 * k2 - epsilon5]]).stream(pointStream);
    return reset();
  };
...
function mercatorProjection(project2) {
  ...
  function reclip() {
    var k2 = pi3 * scale3(), t = m(rotation_default(m.rotate()).invert([0, 0]));
...
var Path2 = class {
  ...
  arcTo(x12, y12, x22, y22, r) {
    ...
    if (this._x1 === null) {
      ...
      let x20 = x22 - x06, y20 = y22 - y06, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi4 - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
  ...
  arc(x3, y3, r, a0, a1, ccw) {
    ...
    if (da2 > tauEpsilon) {
      ...
      this._append`A${r},${r},0,${+(da2 >= pi4)},${cw},${this._x1 = x3 + r * Math.cos(a1)},${this._y1 = y3 + r * Math.sin(a1)}`;
    }
  }
  rect(x3, y3, w, h) {
...
function line_default2(x3, y3) {
  var defined2 = constant_default9(true), context = null, curve = linear_default, output = null, path2 = withPath(line2);
  x3 = typeof x3 === "function" ? x3 : x3 === void 0 ? x2 : constant_default9(x3);
  y3 = typeof y3 === "function" ? y3 : y3 === void 0 ? y2 : constant_default9(y3);
  function line2(data) {
  ...
  line2.x = function(_) {
    return arguments.length ? (x3 = typeof _ === "function" ? _ : constant_default9(+_), line2) : x3;
  };
  line2.y = function(_) {
    return arguments.length ? (y3 = typeof _ === "function" ? _ : constant_default9(+_), line2) : y3;
  };
  line2.defined = function(_) {
    return arguments.length ? (defined2 = typeof _ === "function" ? _ : constant_default9(!!_), line2) : defined2;
  };
...
function area_default5(x06, y06, y12) {
  var x12 = null, defined2 = constant_default9(true), context = null, curve = linear_default, output = null, path2 = withPath(area2);
  x06 = typeof x06 === "function" ? x06 : x06 === void 0 ? x2 : constant_default9(+x06);
  y06 = typeof y06 === "function" ? y06 : y06 === void 0 ? constant_default9(0) : constant_default9(+y06);
  y12 = typeof y12 === "function" ? y12 : y12 === void 0 ? y2 : constant_default9(+y12);
  function area2(data) {
  ...
  area2.x = function(_) {
    return arguments.length ? (x06 = typeof _ === "function" ? _ : constant_default9(+_), x12 = null, area2) : x06;
  };
  area2.x0 = function(_) {
    return arguments.length ? (x06 = typeof _ === "function" ? _ : constant_default9(+_), area2) : x06;
  };
  area2.x1 = function(_) {
    return arguments.length ? (x12 = _ == null ? null : typeof _ === "function" ? _ : constant_default9(+_), area2) : x12;
  };
  area2.y = function(_) {
    return arguments.length ? (y06 = typeof _ === "function" ? _ : constant_default9(+_), y12 = null, area2) : y06;
  };
  area2.y0 = function(_) {
    return arguments.length ? (y06 = typeof _ === "function" ? _ : constant_default9(+_), area2) : y06;
  };
  area2.y1 = function(_) {
    return arguments.length ? (y12 = _ == null ? null : typeof _ === "function" ? _ : constant_default9(+_), area2) : y12;
  };
  ...
  area2.defined = function(_) {
    return arguments.length ? (defined2 = typeof _ === "function" ? _ : constant_default9(!!_), area2) : defined2;
  };
...
// node_modules/d3/node_modules/d3-shape/src/symbol/asterisk.js
var sqrt3 = sqrt2(3);
...
// node_modules/d3/node_modules/d3-shape/src/symbol/diamond2.js
...
// node_modules/d3/node_modules/d3-shape/src/symbol/plus.js
var plus_default = {
  draw(context, size) {
...
// node_modules/d3/node_modules/d3-shape/src/symbol/square2.js
var square2_default = {
  draw(context, size) {
...
// node_modules/d3/node_modules/d3-shape/src/symbol/triangle2.js
var sqrt33 = sqrt2(3);
...
// node_modules/d3/node_modules/d3-shape/src/symbol/times.js
var times_default = {
  draw(context, size) {
...
MonotoneX.prototype = {
  ...
  point: function(x3, y3) {
    ...
    switch (this._point) {
      ...
      case 2:
        this._point = 3;
        point5(this, slope2(this, t1 = slope3(this, x3, y3)), t1);
        break;
      default:
        point5(this, this._t0, t1 = slope3(this, x3, y3));
        break;
    }
    this._x0 = this._x1, this._x1 = x3;
    this._y0 = this._y1, this._y1 = y3;
...
function parseTimeInterval(input) {
  ...
  if (period2 > 1 && !interval2.every) throw new Error(`non-periodic interval: ${name}`);
  return [name, period2];
}
...
function valueof(data, value, type2) {
  ...
  return valueType === "string" ? isArrowTable(data) ? maybeTypedArrowify(data.getChild(value), type2) : maybeTypedMap(data, field(value), type2) : valueType === "function" ? maybeTypedMap(data, value, type2) : valueType === "number" || value instanceof Date || valueType === "boolean" ? map(data, constant(value), type2) : typeof (value == null ? void 0 : value.transform) === "function" ? maybeTypedArrayify(value.transform(data), type2) : maybeTake(maybeTypedArrayify(value, type2), data == null ? void 0 : data[reindex]);
}
function maybeTake(values2, index) {
  return values2 != null && index ? take(values2, index) : values2;
}
function maybeTypedMap(data, f, type2) {
  return map(data, isNumberType(type2) ? (d, i) => coerceNumber(f(d, i)) : f, type2);
}
function maybeTypedArrayify(data, type2) {
  return type2 === void 0 ? arrayify(data) : isArrowVector(data) ? maybeTypedArrowify(data, type2) : data instanceof type2 ? data : type2.from(data, isNumberType(type2) && !isNumberArray(data) ? coerceNumber : void 0);
}
...
function maybeZero(x3, x12, x22, x32 = identity3) {
...
function mid(x12, x22) {
  return {
    transform(data) {
      ...
      return isTemporal(X13) || isTemporal(X23) ? map(X13, (_, i) => new Date((+X13[i] + +X23[i]) / 2)) : map(X13, (_, i) => (+X13[i] + +X23[i]) / 2, Float64Array);
    },
    label: x12.label
  };
}
...
function maybeInterval(interval2, type2) {
  ...
  if (typeof interval2 === "string") return (type2 === "time" ? timeInterval : utcInterval)(interval2);
...
function basic({ filter: f1, sort: s1, reverse: r1, transform: t1, initializer: i1, ...options } = {}, transform2) {
  if (t1 === void 0) {
...
function sortValue(value) {
  ...
  if (order === void 0) order = negate2 ? descendingDefined : ascendingDefined;
...
function groupn(x3, y3, {
  ...
  reduceData = maybeGroupReduce(reduceData, identity3);
        ...
        for (const [f, I] of maybeGroup(facet, G)) {
          for (const [y4, gg] of maybeGroup(I, Y3)) {
            for (const [x4, g] of maybeGroup(gg, X3)) {
              ...
              if (Y3) extent3.y = y4;
              if (G) extent3.z = f;
              if (filter3 && !filter3.reduce(g, extent3)) continue;
              groupFacet.push(i++);
...
var reduceTitle = {
  reduceIndex(I, X3) {
    ...
    const top2 = groups.slice(-n).reverse();
    if (top2.length < groups.length) {
...
function createChannel(data, { scale: scale3, type: type2, value, filter: filter3, hint, label = labelof(value) }, name) {
...
function valueObject(channels, scales) {
  const values2 = Object.fromEntries(
    Object.entries(channels).map(([name, { scale: scaleName, value }]) => {
      ...
      return [name, scale3 == null ? value : map(value, scale3)];
    })
  );
...
function difference(channels, k1, k2) {
  ...
  return map(X23, (x22, i) => Math.abs(x22 - X13[i]), Float64Array);
}
...
function createScaleQ(key, scale3, channels, {
  ...
  if (interpolate.length === 1) {
    ...
    scale3.interpolate((range3 === unit ? constant : interpolatePiecewise)(interpolate));
  } else {
...
function createScaleQuantize(key, channels, {
  ...
  if (range3 === void 0) {
    ...
    if (thresholds[thresholds.length - 1] >= max5) thresholds.pop();
...
function createScaleD(key, scale3, transform2, channels, {
  ...
  if (descending(min4, max5) < 0) [min4, max5] = [max5, min4], reverse3 = !reverse3;
  ...
  if (symmetric) {
    ...
    else if (mindelta > maxdelta) max5 = transform2.invert(mid2 + mindelta);
  }
...
function piecewiseRange(scale3) {
  const length2 = scale3.scale.domain().length + isThresholdScale(scale3);
  if (!(length2 > 2)) return scale3.range;
...
function coerceType(channels, { domain, ...options }, coerceValues) {
  ...
  for (const c4 of channels) {
    if (c4.value !== void 0) {
      if (domain === void 0) domain = (_a = c4.value) == null ? void 0 : _a.domain;
...
function facetTranslator(fx, fy, { marginTop, marginLeft }) {
  ...
  return function(d) {
    if (this.tagName === "svg") {
      this.setAttribute("x", x3(d));
      this.setAttribute("y", y3(d));
    } else {
      this.setAttribute("transform", `translate(${x3(d)},${y3(d)})`);
    }
  };
}
...
function facetIndex(V) {
  ...
  if (!I) indexCache.set(V, I = new InternMap(map(V, (v2, i) => [v2, i])));
  return I;
}
...
function facetFind(facets, x3, y3) {
  x3 = keyof(x3);
  y3 = keyof(y3);
  return facets.find((f) => Object.is(keyof(f.x), x3) && Object.is(keyof(f.y), y3));
}
...
function facetAnchorTopEmpty(facets, { y: Y3 }, { x: x3, y: y3, empty: empty2 }) {
  if (empty2) return false;
  if (!Y3) return;
  const i = facetIndexOf(Y3, y3);
  if (i > 0) return facetEmpty(facets, x3, Y3[i - 1]);
}
function facetAnchorBottomEmpty(facets, { y: Y3 }, { x: x3, y: y3, empty: empty2 }) {
  if (empty2) return false;
  if (!Y3) return;
  const i = facetIndexOf(Y3, y3);
  if (i < Y3.length - 1) return facetEmpty(facets, x3, Y3[i + 1]);
}
function facetAnchorLeftEmpty(facets, { x: X3 }, { x: x3, y: y3, empty: empty2 }) {
  if (empty2) return false;
  if (!X3) return;
  const i = facetIndexOf(X3, x3);
  if (i > 0) return facetEmpty(facets, X3[i - 1], y3);
}
function facetAnchorRightEmpty(facets, { x: X3 }, { x: x3, y: y3, empty: empty2 }) {
  if (empty2) return false;
  if (!X3) return;
  const i = facetIndexOf(X3, x3);
...
function facetFilter(facets, { channels: { fx, fy }, groups }) {
  return fx && fy ? facets.map(({ x: x3, y: y3 }) => {
    var _a;
    return ((_a = groups.get(x3)) == null ? void 0 : _a.get(y3)) ?? [];
  }) : fx ? facets.map(({ x: x3 }) => groups.get(x3) ?? []) : facets.map(({ y: y3 }) => groups.get(y3) ?? []);
}
...
function createProjection({
  ...
  let transform2;
  if (domain != null) {
    const [[x06, y06], [x12, y12]] = path_default2(projection3).bounds(domain);
  ...
  transform2 ?? (transform2 = tx === 0 && ty === 0 ? identity4() : transform_default({
    point(x3, y3) {
...
function scaleProjection(createProjection2, kx2, ky2) {
  return {
    type: ({ width, height, rotate, precision = 0.15, clip }) => {
      ...
      if (precision != null) (_a = projection3.precision) == null ? void 0 : _a.call(projection3, precision);
      if (rotate != null) (_b = projection3.rotate) == null ? void 0 : _b.call(projection3, rotate);
      if (typeof clip === "number") (_c = projection3.clipAngle) == null ? void 0 : _c.call(projection3, clip);
      if (width != null) {
...
var identity4 = constant({ stream: (stream) => stream });
...
function projectionAspectRatio(projection3) {
  if (typeof (projection3 == null ? void 0 : projection3.stream) === "function") return defaultAspectRatio;
  if (isObject(projection3)) {
    let domain, options;
    ...
    if (domain != null && projection3 != null) {
      ...
      const [[x06, y06], [x12, y12]] = path_default2(type2({ ...options, width: 100, height: 100 })).bounds(domain);
...
function applyPosition(channels, scales, { projection: projection3 }) {
  ...
  if (projection3 && (x3 == null ? void 0 : x3.scale) === "x" && (y3 == null ? void 0 : y3.scale) === "y") project("x", "y", position3, projection3);
...
function formatAuto(locale = "en-US") {
  const number5 = formatNumber(locale);
  return (v2) => (v2 instanceof Date ? formatIsoDate : typeof v2 === "number" ? number5 : string)(v2);
}
...
function* groupIndex(I, position3, mark, channels) {
  ...
  for (const G of Z ? groupZ2(I, Z, z) : [I]) {
    ...
    out: for (const i of G) {
      ...
      if (Ag === void 0) {
        if (Gg) yield Gg;
        Ag = A.map((c4) => keyof(c4[i])), Gg = [i];
        continue;
      }
      Gg.push(i);
      for (let j = 0; j < A.length; ++j) {
        const k2 = keyof(A[j][i]);
        if (k2 !== Ag[j]) {
          yield Gg;
          Ag = A.map((c4) => keyof(c4[i])), Gg = [i];
          continue out;
...
function impliedNumber(value, impliedValue) {
  if ((value = number3(value)) !== impliedValue) return value;
}
...
function pointerK(kx2, ky2, { x: x3, y: y3, px, py, maxRadius = 40, channels, render, ...options } = {}) {
  maxRadius = +maxRadius;
  if (px != null) x3 ?? (x3 = null), channels = { ...channels, px: { value: px, scale: "x" } };
  if (py != null) y3 ?? (y3 = null), channels = { ...channels, py: { value: py, scale: "y" } };
  return {
    x: x3,
    y: y3,
    channels,
    ...
    render: composeRender(function(index, scales, values2, dimensions, context, next) {
      ...
      if (x4 == null ? void 0 : x4.bandwidth) tx += x4.bandwidth() / 2;
      if (y4 == null ? void 0 : y4.bandwidth) ty += y4.bandwidth() / 2;
...
:where(.${className}-ramp text) {
  ...
  if (interpolate) {
    ...
    x3 = applyRange(linear().domain([-1, range3.length - 1]), [marginLeft, width - marginRight]);
  ...
  svg2.append("g").attr("transform", `translate(0,${height - marginBottom})`).call(
    axisBottom(x3).ticks(Array.isArray(ticks2) ? null : ticks2, typeof tickFormat === "string" ? tickFormat : void 0).tickFormat(typeof tickFormat === "function" ? tickFormat : void 0).tickSize(tickSize).tickValues(Array.isArray(ticks2) ? ticks2 : null)
...
function applyMarkersColor(path2, { markerStart, markerMid, markerEnd, stroke }, strokeof = () => stroke, Z, context) {
  ...
  function applyMarker(name, marker, filter3) {
    return function(i) {
...
function maybeIntervalMidK(k2, maybeInsetK, options) {
  ...
  return maybeInsetK({
    ...
    [k2]: {
      label: labelof(v2),
      transform: (data) => {
        const V1 = map(valueof(data, value), (v3) => interval2.floor(v3));
...
var RuleX = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x3 }, offset, 0).call(
      (g) => g.selectAll().data(index).enter().append("line").call(applyDirectStyles, this).attr("x1", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("x2", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("y1", Y13 && !isCollapsed(y3) ? (i) => Y13[i] + insetTop : marginTop + insetTop).attr(
        "y2",
...
var RuleY = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { y: Y3 && y3 }, 0, offset).call(
      (g) => g.selectAll().data(index).enter().append("line").call(applyDirectStyles, this).attr("x1", X13 && !isCollapsed(x3) ? (i) => X13[i] + insetLeft : marginLeft + insetLeft).attr(
        "x2",
...
function ruleX(data, options) {
  let { x: x3 = identity3, y: y3, y1: y12, y2: y22, ...rest } = maybeIntervalY(options);
  [y12, y22] = maybeOptionalZero(y3, y12, y22);
  return new RuleX(data, { ...rest, x: x3, y1: y12, y2: y22 });
}
function ruleY(data, options) {
  let { y: y3 = identity3, x: x3, x1: x12, x2: x22, ...rest } = maybeIntervalX(options);
...
var Text = class extends Mark {
  constructor(data, options = {}) {
    const {
      x: x3,
      y: y3,
      text: text2 = isIterable(data) && isTextual(data) ? identity3 : indexOf,
      frameAnchor,
...
function textX(data, { x: x3 = identity3, ...options } = {}) {
  return new Text(data, maybeIntervalMidY({ ...options, x: x3 }));
}
function textY(data, { y: y3 = identity3, ...options } = {}) {
...
var Vector = class extends Mark {
  constructor(data, options = {}) {
    const { x: x3, y: y3, r = defaultRadius, length: length2, rotate, shape = shapeArrow, anchor = "middle", frameAnchor } = options;
  ...
  render(index, scales, channels, dimensions, context) {
    const { x: x3, y: y3 } = scales;
    const { x: X3, y: Y3, length: L, rotate: A } = channels;
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x3, y: Y3 && y3 }).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr(
        "transform",
        template`translate(${X3 ? (i) => X3[i] : cx},${Y3 ? (i) => Y3[i] : cy})${A ? (i) => ` rotate(${A[i]})` : rotate ? ` rotate(${rotate})` : ``}${anchor === "start" ? `` : anchor === "end" ? L ? (i) => ` translate(0,${L[i]})` : ` translate(0,${length2})` : L ? (i) => ` translate(0,${L[i] / 2})` : ` translate(0,${length2 / 2})`}`
      ).attr(
        "d",
        L ? (i) => {
          const p = pathRound();
...
function vectorX(data, options = {}) {
  const { x: x3 = identity3, ...rest } = options;
  return new Vector(data, { ...rest, x: x3 });
}
function vectorY(data, options = {}) {
  const { y: y3 = identity3, ...rest } = options;
...
function axisKy(k2, anchor, data, {
  ...
  tickSize = number3(tickSize);
  tickPadding = number3(tickPadding);
  tickRotate = number3(tickRotate);
...
function axisTextKy(k2, anchor, data, {
  ...
  tickSize,
  tickRotate = 0,
  tickPadding = Math.max(3, 9 - tickSize) + (Math.abs(tickRotate) > 60 ? 4 * Math.cos(tickRotate * radians2) : 0),
  text: text2,
  ...
  return axisMark(
    ...
    function(scale3, data2, ticks2, tickFormat, channels) {
      if (fontVariant === void 0) this.fontVariant = inferFontVariant3(scale3);
      if (text2 === void 0) channels.text = inferTextChannel(scale3, data2, ticks2, tickFormat, anchor);
    }
  );
}
function axisTextKx(k2, anchor, data, {
  ...
  tickSize,
  tickRotate = 0,
  tickPadding = Math.max(3, 9 - tickSize) + (Math.abs(tickRotate) >= 10 ? 4 * Math.cos(tickRotate * radians2) : 0),
  text: text2,
...
function gridKy(k2, anchor, data, {
  ...
  return axisMark(ruleY, k2, data, { ariaLabel, ariaHidden }, { y: y3, x1: x12, x2: x22, ...gridDefaults(options) });
}
function gridKx(k2, anchor, data, {
  ...
  return axisMark(ruleX, k2, data, { ariaLabel, ariaHidden }, { x: x3, y1: y12, y2: y22, ...gridDefaults(options) });
}
...
function axisMark(mark, k2, data, properties, options, initialize) {
  ...
  function axisInitializer(data2, facets, _channels, scales, dimensions, context) {
    ...
    const domain = scale3.domain();
    let { interval: interval2, ticks: ticks2, tickFormat, tickSpacing = k2 === "x" ? 80 : 35 } = options;
    ...
    if (data2 == null) {
      ...
      if (k2 === "y" || k2 === "x") {
        ...
        channels[k2] = { scale: k2, value: identity3 };
      }
    }
    initialize == null ? void 0 : initialize.call(this, scale3, data2, ticks2, tickFormat, channels);
...
function inferTextChannel(scale3, data, ticks2, tickFormat, anchor) {
  return { value: inferTickFormat(scale3, data, ticks2, tickFormat, anchor) };
}
function inferTickFormat(scale3, data, ticks2, tickFormat, anchor) {
  return typeof tickFormat === "function" && !(scale3.type === "log" && scale3.tickFormat) ? tickFormat : tickFormat === void 0 && data && isTemporal(data) ? inferTimeFormat(scale3.type, data, anchor) ?? formatDefault : scale3.tickFormat ? scale3.tickFormat(typeof ticks2 === "number" ? ticks2 : null, tickFormat) : typeof tickFormat === "string" && scale3.domain().length > 0 ? (isTemporal(scale3.domain()) ? utcFormat : format)(tickFormat) : tickFormat === void 0 ? formatDefault : constant(tickFormat);
}
...
function legendItems(scale3, options = {}, swatch) {
  ...
  const context = createContext(options);
  className = maybeClassName(className);
  tickFormat = inferTickFormat(scale3.scale, scale3.domain, void 0, tickFormat);
...
function legendOptions({ className, ...context }, { label, ticks: ticks2, tickFormat } = {}, options) {
  return inherit2(options, { className, ...context }, { label, ticks: ticks2, tickFormat });
}
...
function maybeIdentityX(options = {}, k2 = "x") {
  return hasX(options) ? options : { ...options, [k2]: identity3 };
}
function maybeIdentityY(options = {}, k2 = "y") {
  return hasY(options) ? options : { ...options, [k2]: identity3 };
}
...
function exclusiveFacets(data, facets) {
  ...
  facets = facets.map((facet) => slice4(facet, Uint32Array));
  let j = n;
  O.fill(0);
  for (const facet of facets) {
...
function stack(x3, y3 = one, kx2, ky2, { offset: offset2, order, reverse: reverse3 }, options) {
...
function offsetWiggle(facetstacks, Y13, Y23, Z) {
  for (const stacks of facetstacks) {
    ...
    for (const stack2 of stacks) {
      ...
      const s1 = sum(Fi);
      if (s1) y3 -= sum(Fi, (d, i) => (Df[i] / 2 + Cf1[i]) * d) / s1;
    }
    offsetZero(stacks, Y13, Y23);
  }
  offsetCenterFacets(facetstacks, Y13, Y23);
...
function maybeOrder2(order, offset2, ky2) {
  ...
  if (typeof order === "string") {
    ...
    const compare = negate2 ? descendingDefined : ascendingDefined;
...
var Rect = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    const bx = (x3 == null ? void 0 : x3.bandwidth) ? x3.bandwidth() : 0;
    const by = (y3 == null ? void 0 : y3.bandwidth) ? y3.bandwidth() : 0;
...
function rectInsets(mark, { inset = 0, insetTop = inset, insetRight = inset, insetBottom = inset, insetLeft = inset } = {}) {
  mark.insetTop = number3(insetTop);
  mark.insetRight = number3(insetRight);
  mark.insetBottom = number3(insetBottom);
  mark.insetLeft = number3(insetLeft);
}
...
function rectX(data, options = {}) {
  if (!hasXY(options)) options = { ...options, y: indexOf, x2: identity3, interval: 1 };
  return new Rect(data, maybeStackX(maybeTrivialIntervalY(maybeIdentityX(options))));
}
function rectY(data, options = {}) {
  if (!hasXY(options)) options = { ...options, x: indexOf, y2: identity3, interval: 1 };
...
var Tip = class extends Mark {
  constructor(data, options = {}) {
    ...
    if (options.title === void 0 && isIterable(data) && isTextual(data)) options = { ...options, title: identity3 };
    const {
      x: x3,
      y: y3,
      x1: x12,
    ...
    this.monospace = !!monospace;
    this.fontFamily = string(fontFamily);
    this.fontSize = number3(fontSize);
    this.fontStyle = string(fontStyle);
    this.fontVariant = string(fontVariant);
    this.fontWeight = string(fontWeight);
...
function plot(options = {}) {
  ...
  context.path = function() {
    return path_default2(this.projection ?? xyProjection(scales));
  };
...
function binX(outputs = { y: "count" }, options = {}) {
  ...
  return binn(maybeBinValue(x3, options, identity3), null, null, y3, outputs, maybeInsetX(options));
}
function binY(outputs = { x: "count" }, options = {}) {
  [outputs, options] = mergeOptions2(outputs, options);
  const { x: x3, y: y3 } = options;
  return binn(null, maybeBinValue(y3, options, identity3), x3, null, outputs, maybeInsetY(options));
}
...
function maybeDenseInterval(bin2, k2, options = {}) {
...
function binn(bx, by, gx, gy, {
  ...
  reduceData = maybeBinReduce(reduceData, identity3);
  ...
  return {
    ...
    ..."stroke" in inputs && { stroke: GS || stroke },
    ...basic(options, (data, facets, plotOptions) => {
      const K = maybeApplyInterval(valueof(data, k2), plotOptions == null ? void 0 : plotOptions[gk]);
      const Z = valueof(data, z);
      const F = valueof(data, vfill);
      const S = valueof(data, vstroke);
      const G = maybeSubgroup(outputs, { z: Z, fill: F, stroke: S });
      ...
      for (const facet of facets) {
        ...
        if (filter3) filter3.scope("facet", facet);
        for (const [f, I] of maybeGroup(facet, G)) {
          for (const [k3, g] of maybeGroup(I, K)) {
            for (const [b, extent3] of bin2(g)) {
              if (G) extent3.z = f;
              if (filter3 && !filter3.reduce(b, extent3)) continue;
              groupFacet.push(i++);
...
function maybeBin(options) {
  ...
  const bin2 = (data) => {
    ...
    if (isTemporal(V) || isTimeThresholds(thresholds)) {
      ...
      if (typeof t === "number") t = utcTickInterval(min4, max5, t);
      if (isInterval(t)) {
        if (domain === extent) {
          min4 = t.floor(min4);
...
function bin1(E, T, V) {
  ...
  return (I) => {
    var _a;
    const B2 = E.map(() => []);
    for (const i of I) (_a = B2[bisect_default(T, V[i]) - 1]) == null ? void 0 : _a.push(i);
    return B2;
  };
}
...
var Area = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales, 0, 0).call(
      (g) => g.selectAll().data(groupIndex(index, [X13, Y13, X23, Y23], this, channels)).enter().append("path").call(applyDirectStyles, this).call(applyGroupedChannelStyles, this, channels).attr(
        "d",
...
var Link = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr(
        "d",
        curve === curveAuto && context.projection ? sphereLink(context.path(), X13, Y13, X23, Y23) : (i) => {
          const p = pathRound();
          const c4 = curve(p);
          c4.lineStart();
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr("d", (i) => {
        ...
        const bendAngle = this.sweep(x12, y12, x22, y22) * bend * radians2;
...
var AbstractBar = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(this._transform, this, scales).call(
      (g) => g.selectAll().data(index).enter().call(
        rx1y1 || rx1y2 || rx2y1 || rx2y2 ? (g2) => g2.append("path").call(applyDirectStyles, this).call(applyRoundedRect, x3, y3, add2(x3, w), add2(y3, h), this).call(applyChannelStyles, this, channels) : (g2) => g2.append("rect").call(applyDirectStyles, this).attr("x", x3).attr("width", w).attr("y", y3).attr("height", h).call(applyAttr, "rx", rx).call(applyAttr, "ry", ry).call(applyChannelStyles, this, channels)
      )
    ).node();
  }
...
function barX(data, options = {}) {
  if (!hasXY(options)) options = { ...options, y: indexOf, x2: identity3 };
  return new BarX(data, maybeStackX(maybeIntervalX(maybeIdentityX(options))));
}
function barY(data, options = {}) {
  if (!hasXY(options)) options = { ...options, x: indexOf, y2: identity3 };
...
function cellX(data, { x: x3 = indexOf, fill, stroke, ...options } = {}) {
  if (fill === void 0 && maybeColorChannel(stroke)[0] === void 0) fill = identity3;
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x3, y: Y3 && y3 }).call(
      (g) => g.selectAll().data(index).enter().append(circle2 ? "circle" : "path").call(applyDirectStyles, this).call(
        circle2 ? (selection2) => {
          ...
          selection2.attr(
            "transform",
            template`translate(${X3 ? (i) => X3[i] : cx},${Y3 ? (i) => Y3[i] : cy})${A ? (i) => ` rotate(${A[i]})` : rotate ? ` rotate(${rotate})` : ``}`
          ).attr(
            "d",
            R && S ? (i) => {
              const p = pathRound();
...
function dotX(data, { x: x3 = identity3, ...options } = {}) {
  return new Dot(data, maybeIntervalMidY({ ...options, x: x3 }));
}
function dotY(data, { y: y3 = identity3, ...options } = {}) {
...
var Line = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    const { curve } = this;
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(groupIndex(index, [X3, Y3], this, channels)).enter().append("path").call(applyDirectStyles, this).call(applyGroupedChannelStyles, this, channels).call(applyGroupedMarkers, this, channels, context).attr(
        "d",
...
function lineX(data, { x: x3 = identity3, y: y3 = indexOf, ...options } = {}) {
  return new Line(data, maybeDenseIntervalY({ ...options, x: x3, y: y3 }));
}
function lineY(data, { x: x3 = indexOf, y: y3 = identity3, ...options } = {}) {
...
function autoSpec(data, options) {
  ...
  const C = materializeValue(data, color3);
...
function mapX(mapper, options = {}) {
  ...
  if (x3 === void 0 && x12 === void 0 && x22 === void 0) options = { ...options, x: x3 = identity3 };
  const outputs = {};
...
function mapY(mapper, options = {}) {
  ...
  if (y3 === void 0 && y12 === void 0 && y22 === void 0) options = { ...options, y: y3 = identity3 };
  const outputs = {};
    ...
    ...basic(options, (data, facets) => {
      ...
      for (const facet of facets) {
        for (const I of Z ? group(facet, (i) => Z[i]).values() : [facet]) {
          channels.forEach(({ map: map3 }, i) => map3.mapIndex(I, X3[i], MX[i]));
        }
      }
      return { data, facets };
    }),
...
function reduceArray(f) {
  return (k2, s2, strict) => strict ? {
    mapIndex(I, S, T) {
      let count3 = 0;
      for (let i = 0; i < k2 - 1; ++i) count3 += defined(S[I[i]]);
      for (let i = 0, n = I.length - k2 + 1; i < n; ++i) {
        count3 += defined(S[I[i + k2 - 1]]);
        if (count3 === k2) T[I[i + s2]] = f(subarray(I, i, i + k2), S);
  ...
  if (strict) {
    ...
    return {
      mapIndex(I, S, T) {
        ...
        for (let i = 0, j = Math.min(n, k2 - s2 - 1); i < j; ++i) {
          let v2 = S[I[i]];
          if (v2 !== null && !isNaN(v2 = +v2)) sum4 += v2, ++count3;
        }
        for (let i = -s2, j = n - s2; i < j; ++i) {
          let a2 = S[I[i + k2 - 1]];
          let b = S[I[i]];
          if (a2 !== null && !isNaN(a2 = +a2)) sum4 += a2, ++count3;
          T[I[i + s2]] = sum4 / count3;
          if (b !== null && !isNaN(b = +b)) sum4 -= b, --count3;
        }
      }
    };
  }
...
function bollingerX(data, {
  ...
  return marks(
    ...
    isNoneish(stroke) ? null : lineX(data, map2({ x: bollinger(options) }, { x: x3, y: y3, stroke, strokeOpacity, strokeWidth, ...options }))
  );
}
function bollingerY(data, {
  ...
  return marks(
    ...
    isNoneish(stroke) ? null : lineY(data, map2({ y: bollinger(options) }, { x: x3, y: y3, stroke, strokeOpacity, strokeWidth, ...options }))
  );
}
...
function tickX(data, { x: x3 = identity3, ...options } = {}) {
  return new TickX(data, { ...options, x: x3 });
}
function tickY(data, { y: y3 = identity3, ...options } = {}) {
...
function boxX(data, {
  ...
  return marks(
    ...
    dot(data, map2({ x: oqr }, { x: x3, y: y3, z: y3, r, stroke, strokeOpacity, ...options }))
  );
}
function boxY(data, {
  y: y3 = identity3,
  ...
  return marks(
    ...
    dot(data, map2({ y: oqr }, { x: x3, y: y3, z: x3, r, stroke, strokeOpacity, ...options }))
  );
}
...
var AbstractRaster = class extends Mark {
  constructor(data, channels, options = {}, defaults22) {
    ...
    if (x12 != null) x12 = number4(x12, "x1");
    if (y12 != null) y12 = number4(y12, "y1");
    if (x22 != null) x22 = number4(x22, "x2");
    if (y22 != null) y22 = number4(y22, "y2");
    ...
    this.width = width;
    this.height = height;
    this.pixelSize = number4(pixelSize, "pixelSize");
    this.blur = number4(blur, "blur");
...
var Raster = class extends AbstractRaster {
  ...
  render(index, scales, values2, dimensions, context) {
    ...
    if (this.interpolate) {
      const kx2 = w / dx;
      const ky2 = h / dy;
      const IX = map(X3, (x3) => (x3 - x12) * kx2, Float64Array);
      const IY = map(Y3, (y3) => (y3 - y12) * ky2, Float64Array);
...
function raster() {
  ...
  return new Raster(
    data,
    data == null || options.fill !== void 0 || options.fillOpacity !== void 0 ? options : { ...options, fill: identity3 }
  );
}
...
function interpolateNone(index, width, height, X3, Y3, V) {
  const W = new Array(width * height);
  for (const i of index) {
    if (X3[i] < 0 || X3[i] >= width || Y3[i] < 0 || Y3[i] >= height) continue;
    W[Math.floor(Y3[i]) * width + Math.floor(X3[i])] = V[i];
  }
  return W;
}
function interpolatorBarycentric({ random = lcg(42) } = {}) {
  return (index, width, height, X3, Y3, V) => {
    ...
    extrapolateBarycentric(W, S, X3, Y3, V, width, height, hull2, index, mix);
    return W;
  };
}
function extrapolateBarycentric(W, S, X3, Y3, V, width, height, hull2, index, mix) {
  X3 = Float64Array.from(hull2, (i) => X3[index[i]]);
  Y3 = Float64Array.from(hull2, (i) => Y3[index[i]]);
...
function interpolateNearest(index, width, height, X3, Y3, V) {
  ...
  return (index, width, height, X3, Y3, V) => {
    ...
    for (let y3 = 0.5, k2 = 0; y3 < height; ++y3) {
      ...
      for (let x3 = 0.5; x3 < width; ++x3, ++k2) {
        ...
        let distance;
        let step = 0;
        while ((distance = Math.hypot(X3[index[iw]] - cx, Y3[index[iw]] - cy)) > minDistance && step < maxSteps) {
...
function denseX(x12, x22, width) {
  return {
    transform(data) {
      ...
      const x06 = x12 + kx2 / 2;
      for (let i = 0; i < n; ++i) X3[i] = i % width * kx2 + x06;
      return X3;
    }
  };
}
function denseY(y12, y22, width, height) {
  return {
    transform(data) {
      ...
      const y06 = y12 + ky2 / 2;
      for (let i = 0; i < n; ++i) Y3[i] = Math.floor(i / width) % height * ky2 + y06;
      return Y3;
    }
  };
}
...
var Contour = class extends AbstractRaster {
  constructor(data, { smooth = true, value, ...options } = {}) {
    ...
    if (value != null) {
      const v2 = { transform: (D2) => D2.map((d) => d.value), label: labelof(value) };
      for (const key in channels) {
  ...
  filter(index, { x: x3, y: y3, value, ...channels }, values2) {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call((g) => {
      g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr("d", (i) => path2(G[i])).call(applyChannelStyles, this, channels);
    }).node();
  }
};
...
function maybeTicks(thresholds, V, min4, max5) {
  if (typeof (thresholds == null ? void 0 : thresholds.range) === "function") return thresholds.range(thresholds.floor(min4), max5);
  if (typeof thresholds === "function") thresholds = thresholds(V, min4, max5);
  if (typeof thresholds !== "number") return arrayify(thresholds);
  const tz = ticks(...nice(min4, max5, thresholds), thresholds);
  while (tz[tz.length - 1] >= max5) tz.pop();
...
var Voronoi2 = class extends Mark {
  constructor(data, options = {}) {
    ...
    super(
      ...
      initializer(options, function(data2, facets, channels, scales, dimensions, context) {
        ...
        const C = new Array((X3 ?? Y3).length).fill(null);
        const [cx, cy] = applyFrameAnchor(this, dimensions);
        const xi = X3 ? (i) => X3[i] : constant(cx);
        const yi = Y3 ? (i) => Y3[i] : constant(cy);
        for (let I of facets) {
  ...
  render(index, scales, channels, dimensions, context) {
    const { x: x3, y: y3 } = scales;
    const { x: X3, y: Y3, cells: C } = channels;
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x3, y: Y3 && y3 }).call((g) => {
...
var Density = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, {}).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).call(applyChannelStyles, this, channels).attr("d", (i) => path2(contours[i]))
    ).node();
  }
};
    ...
    if (!(T instanceof TypedArray)) {
      ...
      for (const facetContours of facetsContours) {
        for (const [, contour2] of facetContours) {
          ...
          if (max5 > maxValue) maxValue = max5;
        }
      }
...
function differenceK(k2, data, {
  ...
  y1: y12,
  y2: y22,
  x: x3 = x12 === void 0 && x22 === void 0 ? k2 === "y" ? indexOf : identity3 : void 0,
  y: y3 = y12 === void 0 && y22 === void 0 ? k2 === "x" ? indexOf : identity3 : void 0,
  fill,
  // ignored
  positiveFill = "#3ca951",
  negativeFill = "#4269d0",
...
function clipDifference(k2, positive2) {
  ...
  return (index, scales, channels, dimensions, context, next) => {
    ...
    (positive2 === inferScaleOrder(scales[k2]) < 0 ? K1 : K2).fill(m);
    const oc = next(index, scales, { ...channels, [f2]: F1, [k22]: K2 }, dimensions, context);
...
function centroid({ geometry = identity3, ...options } = {}) {
  ...
  return initializer(
    ...
    (data, facets, channels, scales, dimensions, context) => {
      ...
      for (let i = 0; i < n; ++i) [X3[i], Y3[i]] = centroid2(G[i]);
      return {
        data,
        facets,
...
function geoCentroid({ geometry = identity3, ...options } = {}) {
  const getG = memoize1((data) => valueof(data, geometry));
...
var Geo = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call((g) => {
      g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr("d", R ? (i) => path2.pointRadius(R[i])(G[i]) : (i) => path2(G[i])).call(applyChannelStyles, this, channels);
    }).node();
  }
};
function geo(data, options = {}) {
  if (options.tip && options.x === void 0 && options.y === void 0) options = centroid(options);
  else if (options.geometry === void 0) options = { ...options, geometry: identity3 };
...
function hbin(data, I, X3, Y3, dx) {
  ...
  for (const i of I) {
    ...
    if (Math.abs(py1) * 3 > 1) {
      ...
      if (px1 * px1 + py1 * py1 > px2 * px2 + py2 * py2) pi7 = pi22 + (pj & 1 ? 1 : -1) / 2, pj = pj2;
    }
    const key = `${pi7},${pj}`;
    let bin2 = bins.get(key);
    if (bin2 === void 0) {
      bin2 = { index: [], extent: { data, x: (pi7 + (pj & 1) / 2) * dx + ox, y: pj * dy + oy } };
      bins.set(key, bin2);
    }
    bin2.index.push(i);
  }
...
var Image2 = class extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    const { x: x3, y: y3 } = scales;
    const { x: X3, y: Y3, width: W, height: H, r: R, rotate: A, src: S } = channels;
    const { r, width, height, rotate } = this;
    const [cx, cy] = applyFrameAnchor(this, dimensions);
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x3, y: Y3 && y3 }).call(
      (g) => g.selectAll().data(index).enter().append("image").call(applyDirectStyles, this).attr("x", position2(X3, W, R, cx, width, r)).attr("y", position2(Y3, H, R, cy, height, r)).attr("width", W ? (i) => W[i] : width !== void 0 ? width : R ? (i) => R[i] * 2 : r * 2).attr("height", H ? (i) => H[i] : height !== void 0 ? height : R ? (i) => R[i] * 2 : r * 2).attr("transform", A ? (i) => `rotate(${A[i]})` : rotate ? `rotate(${rotate})` : null).attr("transform-origin", A || rotate ? template`${X3 ? (i) => X3[i] : cx}px ${Y3 ? (i) => Y3[i] : cy}px` : null).call(applyAttr, "href", S ? (i) => S[i] : this.src).call(applyAttr, "preserveAspectRatio", this.preserveAspectRatio).call(applyAttr, "crossorigin", this.crossOrigin).call(applyAttr, "image-rendering", this.imageRendering).call(applyAttr, "clip-path", R ? (i) => `circle(${R[i]}px)` : r !== void 0 ? `circle(${r}px)` : null).call(applyChannelStyles, this, channels)
    ).node();
  }
};
...
function ibetainv(p, a2, b) {
  ...
  for (; j < 10; j++) {
    ...
    if (Math.abs(t) < EPS * x3 && j > 0) break;
  }
  return x3;
}
...
function linearRegressionX(data, { y: y3 = indexOf, x: x3 = identity3, stroke, fill = isNoneish(stroke) ? "currentColor" : stroke, ...options } = {}) {
  return new LinearRegressionX(data, maybeDenseIntervalY({ ...options, x: x3, y: y3, fill, stroke }));
}
function linearRegressionY(data, { x: x3 = indexOf, y: y3 = identity3, stroke, fill = isNoneish(stroke) ? "currentColor" : stroke, ...options } = {}) {
...
function treeNode({
  ...
  return {
    ...
    frameAnchor,
    ...basic(options, (data, facets) => {
      const P = normalize3(valueof(data, path2));
      const X4 = setX([]);
      const Y4 = setY([]);
      let treeIndex = -1;
      const treeData = [];
...
function nodeSort(value) {
  return (a2, b) => ascendingDefined(value(a2), value(b));
}
...
function wafflePolygon(y3, options) {
  ...
  return initializer(waffleRender(options), function(data, facets, channels, scales, dimensions) {
    ...
    const scale3 = unit2 * scaleof(scales.scales[y3]);
    ...
    const transform2 = y3 === "y" ? ([x4, y4]) => [x4 * cx, -y4 * cy] : ([x4, y4]) => [y4 * cy, x4 * cx];
    const mx = typeof x06 === "function" ? (i) => x06(i) - barwidth / 2 : () => x06;
    ...
    const Y3 = new Float64Array(n);
    for (let i = 0; i < n; ++i) {
      P[i] = wafflePoints(round2(Y13[i] / unit2), round2(Y23[i] / unit2), multiple).map(transform2);
      const c4 = P[i].pop();
      X3[i] = c4[ix] + mx(i);
      Y3[i] = c4[iy] + y06;
    }
  ...
  return {
    ...
    render: composeRender(render, function(index, scales, values2, dimensions, context) {
      ...
      return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(this._transform, this, scales).call(
        ...
        (g) => g.selectAll().data(index).enter().append("path").attr("transform", template`translate(${x06},${y06})`).attr("d", (i) => `M${polygon[i].join("L")}Z`).attr("fill", (i) => `url(#${patternId}-${i})`).attr("stroke", this.stroke == null ? null : "none").call(applyChannelStyles, this, { ariaLabel, href, title })
      ).node();
    })
  };
}
...
function dodgeX(dodgeOptions = {}, options = {}) {
  ...
  return dodge("x", "y", anchor, number3(padding), r, options);
}
function dodgeY(dodgeOptions = {}, options = {}) {
  ...
  return dodge("y", "x", anchor, number3(padding), r, options);
}
...
function dodge(y3, x3, anchor, padding, r, options) {
  ...
  return initializer(options, function(data, facets, channels, scales, dimensions, context) {
    ...
    const cr = R ? void 0 : r !== void 0 ? number3(r) : this.r !== void 0 ? this.r : 3;
    if (R) R = valueof(R.value, scales[R.scale] || identity3, Float64Array);
